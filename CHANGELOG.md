...
- **Dependency maintenance**: Bumped `@behindthemusictree/app-kit` to `4.9.1` (from `4.5.3`), and
  `@behindthemusictree/brand` to `12.0.0` (from `11.6.0`) to match the version app-kit now pulls
  transitively. `4.9.1` only bumps app-kit's own `@behindthemusictree/genre-tree-view` catalog pin
  to `1.5.0`; no consumer-facing change here.
